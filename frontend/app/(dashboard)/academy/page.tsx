        ...
        <div>
          <h1 className="page-title">
            <span className="text-gold-gradient">
              {academy?.name || user?.academyName || "Your Academy"}
            </span>
          </h1>
          <p className="text-[var(--text-muted)] text-sm mt-1 capitalize">
            {academy?.plan || "trial"} plan · {academy?.subdomain}
            .chessacademy.pro
          </p>
        </div>
        {liveClasses.length > 0 && (
          <Link
            href={`/classroom/${liveClasses[0].id}`}
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          ...
          <Link
            key={i}
            href={s.href}
            className="stat-card hover:border-[var(--border)] transition-colors"
          >
      ...
      <div className="grid lg:grid-cols-2 gap-5">
        ...
        <div className="card overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--border)]">
            ...
            <Link
              href="/academy/classes"
              className="text-xs text-[var(--amber)] flex items-center gap-1"
            >
