...
export default function AcademySettingsPage() {
  ...
  useEffect(() => {
    if (academy) {
      ...
      setSlug(academy.slug || academy.subdomain || "");
      setCity(academy.settings?.city || "");
      setCountry(academy.settings?.country || "India");
    }
  }, [academy]);

  const { mutateAsync, isPending } = useMutation({
    mutationFn: () =>
      academiesAPI.update(user!.academyId!, {
        name,
        settings: { ...(academy?.settings || {}), city, country, slug },
      }),
    onSuccess: () => {
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p className="text-xs mt-1" style={{ color: "var(--text-muted)" }}>
              {slug || "your-slug"}.chessacademy.pro
            </p>
          </div>
          <div>
        ...
        <div className="pt-2" style={{ borderTop: "1px solid var(--border)" }}>
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="text-xs capitalize mt-0.5 badge badge-gold">
                {academy?.plan || "trial"}
              </div>
            </div>
            <a href="/academy/billing" className="btn-secondary text-sm">
              Upgrade Plan
            </a>
          </div>
        </div>
      ...
      <div className="card p-6 space-y-4">
        ...
        >
          ...
          <p className="text-xs text-red-600 mt-1">
            Permanently deletes all data including students, coaches, and
            classes. This cannot be undone.
          </p>
          <button className="mt-3 btn-danger text-xs py-2 px-4">
            Delete Academy
          </button>
        </div>
      </div>
