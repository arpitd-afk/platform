...
export default function CoachDashboard() {
  ...
  const avgRating = students.length
    ? Math.round(
        students.reduce((s: number, u: any) => s + (u.rating || 1200), 0) /
          students.length,
      )
    : 0;
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(135deg, #FFFCF8 0%, rgba(21,128,61,0.05) 100%)",
        }}
      >
        ...
        <div className="flex-1">
          ...
          <p className="text-sm mt-1" style={{ color: "var(--text-muted)" }}>
            {pendingReview > 0
              ? `${pendingReview} assignment${pendingReview > 1 ? "s" : ""} awaiting review`
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          },
          {
            ...
            value: classrooms.filter((c: any) => c.status !== "cancelled")
              .length,
            icon: Video,
            color: "#15803D",
            href: "/coach/classroom",
          },
          {
