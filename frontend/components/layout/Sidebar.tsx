...
import {
  LayoutDashboard, Users, BookOpen, Trophy, BarChart3, CreditCard, Settings,
  ChevronLeft, ChevronRight, Puzzle, ClipboardList, Calendar, MessageSquare,
  Swords, Award, TrendingUp, Building2, Shield, Crown, LogOut,
  Bell, Video, GraduationCap, Layers, Wallet, UserCheck, Activity, Menu, X
} from 'lucide-react'
...
const NAV: Record<string, NavGroup[]> = {
  ...
  coach: [
    ...
    { group: 'Teaching', items: [
      ...
      { label: 'Assignments',  href: '/coach/assignments',   icon: ClipboardList },
...
function NavLink({ href, icon: Icon, label, collapsed, unread }: any) {
  ...
  const active = href === pathname || (href.length > 10 && pathname.startsWith(href))
  return (
    <Link href={href} title={collapsed ? label : undefined}
      ...
      style={{ background: active ? 'rgba(200,150,30,0.10)' : '', color: active ? 'var(--amber)' : 'var(--text-muted)', fontWeight: active ? 600 : 500 }}
      onMouseEnter={e => { if (!active) { (e.currentTarget as any).style.background = 'var(--bg-hover)'; (e.currentTarget as any).style.color = 'var(--text-mid)' }}}
      onMouseLeave={e => { if (!active) { (e.currentTarget as any).style.background = ''; (e.currentTarget as any).style.color = 'var(--text-muted)' }}}>
      <Icon size={17} className="flex-shrink-0" />
      {!collapsed && <span className="flex-1">{label}</span>}
      {!collapsed && unread > 0 && <span className="w-5 h-5 rounded-full text-white text-[10px] font-bold flex items-center justify-center" style={{ background: 'var(--amber)' }}>{unread > 9 ? '9+' : unread}</span>}
    </Link>
  )
}
...
function SidebarBody({ collapsed, onNav }: { collapsed: boolean; onNav?: () => void }) {
  ...
  return (
    <div className={`flex flex-col h-full ${collapsed ? 'w-[68px]' : 'w-[240px]'} transition-all duration-300 relative`}
      style={{ background: '#FFFCF8', borderRight: '1px solid var(--border)' }}>
      {/* Logo */}
      <div className={`flex items-center gap-3 py-5 ${collapsed ? 'px-3 justify-center' : 'px-4'}`}
        ...
        {!collapsed && <div>
          <div className="font-display font-bold text-sm" style={{ color: 'var(--text)' }}>Chess Academy</div>
          <div className="text-[10px] capitalize font-medium" style={{ color: roleColor }}>{user?.role?.replace('_', ' ')}</div>
        </div>}
      </div>

      {/* Nav */}
      <nav className="flex-1 overflow-y-auto py-3 scrollbar-none px-2">
        <NavLink href="/notifications" icon={Bell} label="Notifications" collapsed={collapsed} unread={unreadCount} />
        {navGroups.map(g => (
          <div key={g.group} className="mb-1">
            {!collapsed && <p className="text-[10px] font-semibold uppercase tracking-wider px-3 mb-1 mt-3" style={{ color: 'var(--text-muted)' }}>{g.group}</p>}
            {g.items.map(item => <div key={item.href} onClick={onNav}><NavLink href={item.href} icon={item.icon} label={item.label} collapsed={collapsed} unread={0} /></div>)}
          </div>
        ))}
      </nav>
      ...
      <div className="p-3 space-y-1" style={{ borderTop: '1px solid var(--border)' }}>
        <Link href="/profile" onClick={onNav}
          ...
          {!collapsed && <div className="flex-1 min-w-0">
            ...
            <div className="text-[10px]" style={{ color: roleColor }}>{user?.email}</div>
          </div>}
        </Link>
        <button onClick={logout}
          ...
          <LogOut size={15} className="flex-shrink-0" />{!collapsed && 'Sign Out'}
        </button>
      </div>
    </div>
  )
    ...
    <>
      ...
      <div className="hidden lg:flex relative h-screen flex-shrink-0">
        ...
        <button onClick={() => setCollapsed(c => !c)}
          ...
          {collapsed ? <ChevronRight size={13} style={{ color: 'var(--text-muted)' }} /> : <ChevronLeft size={13} style={{ color: 'var(--text-muted)' }} />}
        </button>
      </div>
      ...
      {mobileOpen && (
        <div className="lg:hidden fixed inset-0 z-50 flex">
          <div className="absolute inset-0" style={{ background: 'rgba(28,17,7,0.35)' }} onClick={() => setMobileOpen(false)} />
          <div className="relative h-full shadow-xl">
            <SidebarBody collapsed={false} onNav={() => setMobileOpen(false)} />
            <button onClick={() => setMobileOpen(false)} className="absolute top-4 right-3 btn-icon w-8 h-8"><X size={16} /></button>
          </div>
        </div>
      )}
    </>
