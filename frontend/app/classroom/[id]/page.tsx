...
import { classroomsAPI } from "@/src/lib/api";
...
export default function ClassroomPage() {
  ...
  const isCoach = ["coach", "academy_admin", "super_admin"].includes(
    user?.role || "",
  );
  ...
  const boardStyle = React.useMemo(() => ({ borderRadius: "16px" }), []);
  const darkSquareStyle = React.useMemo(
    () => ({ backgroundColor: "#B58863" }),
    [],
  );
  const lightSquareStyle = React.useMemo(
    () => ({ backgroundColor: "#F0D9B5" }),
    [],
  );
  const dropSquareStyle = React.useMemo(
    () => ({ boxShadow: "inset 0 0 1px 4px rgba(212,175,55,0.6)" }),
    [],
  );

  // Stabilize fallback handler
  const onPieceDropFallback = React.useCallback(() => false, []);
  ...
  useEffect(() => {
    ...
    classroomsAPI
      .get(id)
      .then((r: any) => setClassroom(r.data.classroom))
      .catch(() => {});
  }, [id]);
  ...
  useEffect(() => {
    ...
    // participants join/leave/hand logic
    socket.on("classroom:user_joined", ({ userId, name, role }: any) => {
    ...
    socket.on("classroom:hand_lowered", ({ userId }: any) => {
      setParticipants((prev) =>
        prev.map((p) =>
          p.userId === userId ? { ...p, handRaised: false } : p,
        ),
      );
    });
  ...
  const handleCoachMove = useCallback(
  ...
  const handleArrowDraw = useCallback(
    (newArrows: [string, string][]) => {
      if (!isCoach) return;
      
      // Avoid infinite re-render loop if Chessboard calls this when customArrows prop changes
      if (JSON.stringify(newArrows) === JSON.stringify(arrows)) return;
      
      setArrows(newArrows as any);
  ...
  const clearAnnotations = () => {
    ...
    setAnnotations([]);
    if (token)
      getSocket(token).emit("classroom:clear_annotations", { classroomId: id });
  };

  const sendChat = () => {
    if (!chatMsg.trim() || !token) return;
    getSocket(token).emit("classroom:chat", {
      classroomId: id,
      message: chatMsg,
    });
    setChatMsg("");
  ...
  const savePgn = async () => {
    ...
    try {
      ...
      toast.error("Failed to save PGN");
    }
  };
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-display font-bold text-base">
              {classroom?.title || "Chess Classroom"}
            </h1>
        ...
        <div className="flex-1 flex items-center justify-center p-5">
          <div className="w-full max-w-[600px]">
            ...
            {isCoach && (
              <div className="flex items-center gap-2 mb-3">
                <span className="text-xs text-[var(--text-muted)]">
                  ...
                  Draw arrows by right-click drag on the board
                </span>
                {handsUp.length > 0 && (
                  <div className="ml-auto flex items-center gap-1.5 bg-[#D4AF37]/10 border border-[#D4AF37]/20 rounded-full px-3 py-1 text-xs text-[var(--amber)]">
                    ✋ {handsUp.length} hand{handsUp.length > 1 ? "s" : ""}{" "}
                    raised
                  </div>
                )}
              </div>
            ...
            <div className="rounded-2xl overflow-hidden shadow-2xl">
              <Chessboard
                position={chess.fen()}
                onPieceDrop={isCoach ? handleCoachMove : onPieceDropFallback}
            ...
            {chess.history().length > 0 && (
              <div className="mt-3 flex flex-wrap gap-1">
                {chess.history().map((move, i) => (
                  <span
                    key={i}
                    className="text-xs font-mono bg-[var(--bg-subtle)] px-2 py-1 rounded text-[var(--text-mid)]"
                  >
                    {i % 2 === 0 && (
            ...
            {(
              ...
              >
                ...
                {tab === "students" && handsUp.length > 0 && (
                  <span className="w-4 h-4 rounded-full bg-[#D4AF37] text-[var(--bg)] text-[10px] flex items-center justify-center font-bold">
                    {handsUp.length}
                  </span>
                )}
              </button>
                  ...
                  chat.map((msg) => (
                    ...
                    >
                      ...
                      >
                        ...
                        <div
                          className={`px-3 py-2 rounded-xl text-sm ${msg.userId === user?.id ? "bg-[#D4AF37]/20 text-[var(--text)]" : "bg-[var(--bg-subtle)] text-[var(--text)]"}`}
                        >
                          {msg.message}
                        </div>
                      </div>
