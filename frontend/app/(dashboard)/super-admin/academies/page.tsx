...
function CreateAcademyModal({ onClose }: { onClose: () => void }) {
  ...
  const { mutateAsync, isPending } = useMutation({
    ...
    onError: (e: any) =>
      toast.error(e.response?.data?.message || "Failed to create academy"),
  });
  ...
  const handleSubmit = () => {
    ...
    mutateAsync({ ...form, slug, subdomain: slug });
  };
      ...
      <div className="space-y-5">
        <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                className="input"
                placeholder="Anand Chess Academy"
                value={form.name}
          ...
          <div className="grid grid-cols-2 gap-4 mt-4">
            ...
            <div>
              ...
              <select
                className="input"
                value={form.plan}
                onChange={(e) => up("plan", e.target.value)}
              >
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => up("ownerEmail", e.target.value)}
              />
            </div>
          </div>
          <div className="mt-4">
            ...
            <div className="relative">
              <input
                ...
                onChange={(e) => up("ownerPassword", e.target.value)}
              />
              <button
                type="button"
        ...
        >
          <strong style={{ color: "var(--text-mid)" }}>
            Login credentials for the academy admin:
          </strong>
...
function EditAcademyModal({
  ...
  const [form, setForm] = useState({
    name: academy.name || "",
    plan: academy.plan || "trial",
    city: academy.settings?.city || "",
  });
  const up = (k: string, v: string) => setForm((f) => ({ ...f, [k]: v }));
  const { mutateAsync, isPending } = useMutation({
    mutationFn: () =>
      academiesAPI.update(academy.id, {
        name: form.name,
        settings: { ...(academy.settings || {}), city: form.city },
        plan: form.plan,
      }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["academies"] });
...
export default function AcademiesPage() {
  ...
  const filtered = academies.filter((a: any) => {
    ...
    const matchPlan =
      planFilter === "all" ||
      a.plan === planFilter ||
      (planFilter === "active" && a.is_active) ||
      (planFilter === "suspended" && !a.is_active);
    return matchSearch && matchPlan;
  });

  const handleToggle = async (a: any) => {
    try {
      if (a.is_active) await academiesAPI.suspend(a.id);
      else await academiesAPI.activate(a.id);
      qc.invalidateQueries({ queryKey: ["academies"] });
      toast.success(a.is_active ? "Academy suspended" : "Academy activated");
    } catch {
      toast.error("Failed");
    }
  };
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      {showCreate && (
        <CreateAcademyModal onClose={() => setShowCreate(false)} />
      )}
      {editing && (
        <EditAcademyModal academy={editing} onClose={() => setEditing(null)} />
      )}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          {
            label: "Active",
            value: academies.filter((a: any) => a.is_active).length,
            color: "#15803D",
          },
          {
            label: "Suspended",
          ...
          {
            label: "Enterprise",
            value: academies.filter((a: any) => a.plan === "enterprise").length,
            color: "#7C3AED",
          },
        ].map((s) => (
              ...
              {filtered.map((a: any) => (
                <tr key={a.id} className="tr">
                  <td className="td">
                    ...
                    >
                      <Globe size={10} />
                      {a.subdomain || a.settings?.slug || "—"}.chessacademy.pro
                    </div>
                  </td>
                  <td className="td">
                  ...
                  <td className="td text-center">
                    {a.is_active ? (
                      <span className="badge badge-green text-xs flex items-center justify-center gap-1">
                        <CheckCircle2 size={10} />
                        Active
                      </span>
                    ) : (
                      <span className="badge badge-red text-xs flex items-center justify-center gap-1">
                        <XCircle size={10} />
                        Suspended
                      </span>
                    )}
